...
public interface AuthRoleRepository extends JpaRepository<User, Long>, PagingAndSortingRepository<User, Long> {
    ...
    @Query("SELECT u FROM User u WHERE LOWER(u.username) LIKE LOWER(CONCAT('%', :name, '%')) AND u.role <> :role")
    Page<User> searchByName(@Param("name") String name, @Param("role") String role, Pageable pageable);
